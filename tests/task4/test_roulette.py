...
class TestBet:
    ...
    def test_valid_color_bet_red(self):
        ...
        bet = Bet(BetType.COLOR, Color.RED, 50)
    ...
    def test_valid_color_bet_black(self):
        ...
        bet = Bet(BetType.COLOR, Color.BLACK, 50)
        assert bet.type_bet == BetType.COLOR
        assert bet.bet_value == Color.BLACK

    def test_valid_even_odd_bet(self):
        """Test creating valid even/odd bets"""
        bet_even = Bet(BetType.EVEN_ODD, EvenOdd.EVEN, 50)
        bet_odd = Bet(BetType.EVEN_ODD, EvenOdd.ODD, 50)
        assert bet_even.bet_value == EvenOdd.EVEN
        assert bet_odd.bet_value == EvenOdd.ODD

    def test_valid_dozen_bets(self):
        """Test creating valid dozen bets"""
        for dozen in [Dozen.FIRST, Dozen.SECOND, Dozen.THIRD]:
            bet = Bet(BetType.DOZEN, dozen, 50)
            assert bet.bet_value == dozen

    def test_valid_column_bets(self):
        """Test creating valid column bets"""
        for column in [1, 2, 3]:
            bet = Bet(BetType.COLUMN, column, 50)
            assert bet.bet_value == column

    def test_valid_half_bets(self):
        """Test creating valid half bets"""
        for half in [Half.FIRST_18, Half.LAST_18]:
            bet = Bet(BetType.HALF, half, 50)
    ...
    def test_invalid_number_raises_error(self):
        ...
        with pytest.raises(ValueError, match="Number must be 0-36"):
            Bet(BetType.NUMBER, 37, 100)
        with pytest.raises(ValueError, match="Number must be 0-36"):
            Bet(BetType.NUMBER, -1, 100)

    def test_invalid_color_raises_error(self):
        """Test invalid color raises ValueError"""
        with pytest.raises(ValueError, match="Color must be Color.RED or Color.BLACK"):
            Bet(BetType.COLOR, "green", 100)

    def test_invalid_even_odd_raises_error(self):
        """Test invalid even/odd raises ValueError"""
        with pytest.raises(
            ValueError, match="Even/Odd must be EvenOdd.EVEN or EvenOdd.ODD"
        ):
            Bet(BetType.EVEN_ODD, "invalid", 100)

    def test_invalid_dozen_raises_error(self):
        """Test invalid dozen raises ValueError"""
        with pytest.raises(
            ValueError, match="Dozen must be Dozen.FIRST, Dozen.SECOND or Dozen.THIRD"
        ):
            Bet(BetType.DOZEN, "4th", 100)
    ...
    def test_invalid_column_raises_error(self):
        ...
        with pytest.raises(ValueError, match="Column must be 1, 2 or 3"):
            Bet(BetType.COLUMN, 4, 100)
        with pytest.raises(ValueError, match="Column must be 1, 2 or 3"):
            Bet(BetType.COLUMN, 0, 100)

    def test_invalid_half_raises_error(self):
        """Test invalid half raises ValueError"""
        with pytest.raises(
            ValueError, match="Half must be Half.FIRST_18 or Half.LAST_18"
        ):
            Bet(BetType.HALF, "20-36", 100)
...
class TestRouletteWheel:
    ...
    def test_color_determination_red(self):
        """Test red numbers return correct color"""
        wheel = RouletteWheel()

        for number in wheel.red_numbers:
            assert wheel._get_color(number) == Color.RED

    def test_color_determination_black(self):
        """Test black numbers return correct color"""
        wheel = RouletteWheel()

        for number in wheel.black_numbers:
            assert wheel._get_color(number) == Color.BLACK

    def test_color_determination_zero(self):
        """Test zero returns green"""
        wheel = RouletteWheel()
        assert wheel._get_color(0) == Color.GREEN
...
class TestRouletteGame:
    ...
    def test_is_game_over_continues(self):
        """Test game continues when players have balance and rounds remain"""
        players = [Player(100, 20, ConservativeStrategy())]
        game = RouletteGame(players, max_rounds=5)
...
class TestGameIntegration:
    ...
    def test_multiple_strategies_in_same_game(self):
        """Test game works with multiple different strategies"""
        players = [
    ...
    def test_calculate_winnings_method(self):
        """Test _calculate_winnings() method calculations"""
        game = RouletteGame([], max_rounds=5)

        winning_bet = Bet(BetType.NUMBER, 17, 100)
        winning_result = {
            "number": 17,
            "color": Color.RED,
            "is_even": False,
        ...
        losing_bet = Bet(BetType.NUMBER, 17, 100)
        losing_result = {
            "number": 5,
            "color": Color.RED,
            "is_even": False,
